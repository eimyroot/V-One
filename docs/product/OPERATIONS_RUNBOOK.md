...
The health response must report `database_backend: sqlite` and `schema_version: 7`. Never edit an
...
7. Verify `/api/v1/health` reports `HEALTHY`, `sqlite`, schema version `7`, and production effects
   `DISABLED`.
