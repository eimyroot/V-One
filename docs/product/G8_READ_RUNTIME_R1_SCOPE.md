...
+ closure-pinned role-bound G8 Runner / Verifier READ handlers
+ exact pinned GET-only GitHub provider effect implementation
+ closure-pinned CanonicalGitHubReadTerminal execution bindings
...
The immutable pair transports are retained only behind G8 role-bound Runner and Verifier handlers whose effect-critical transport/fence/clock/revision bindings are closure-pinned. A normal post-build reassignment of either handler's critical binding is rejected. A forced `object.__setattr__` write can alter raw instance storage, but cannot alter the effective execution binding returned to the observation path. The Runner path therefore continues to use the original literal `runner` pair and binds the current `CredentialAccessDecision` credential class to the immutable Runner pin; the Verifier path continues to use the original literal `verifier` pair and binds both current Verifier identity and `VerifierCredentialDecision` credential classes to the immutable Verifier pin.

The canonical READ terminal is assembled through the same pattern: its capability registry, capsule registry, Runner adapter/handler, completion coordinator, Verifier profile/policy/handler/clock and result-revision bindings are closure-pinned, while `run()` delegates through the import-pinned canonical terminal implementation. Post-build writes to raw instance state therefore cannot redirect the effective terminal execution graph.
...
Therefore changing an introspectable closure-registry entry after composition cannot silently replace the Verifier with the Runner credential; reinvoking `__init__` cannot replace a successful binding; post-build rebinding of the module-global provider transport cannot widen the effect implementation; normal handler/terminal rebinding is rejected; forced raw instance-state writes cannot change the effective closure-pinned credential role or terminal execution graph; and validation/provider use cannot diverge through a post-check instance-state change or check/use race.
...
R1 rejects parallel product DBs, parallel permission authority, capsule-registry forks from Grant/conformance authority, foreign current fences, subclasses or instance-level overrides that can replace the durable current-fence implementation, shared Runner/Verifier provider authority, mutation-capable/generic provider transports, collapsed Runner/Verifier credential classes, post-build Runner/Verifier handler-role redirection, product↔Runner environment mismatch, and production widening.
