...
| R-05 | Cancellation is presented as rollback or proof of no effect | HIGH | Cancellation never resets consumption; always run independent postcondition verification after attempted work | Cancel latency, acknowledgement, and post-state audit | External action may complete concurrently; outcome can remain `INDETERMINATE` |
...
| R-10 | Capability exfiltrates data or reaches an unauthorized service | HIGH | Network deny by default; per-capability destination/protocol/port allowlist; minimal data and credentials | Egress deny/allow logs, DNS and destination anomaly alerts, byte budgets | Allowed endpoint may be compromised or used as an exfiltration channel |
...
| R-15 | Receipt is forged, replayed, rebound, conflicted, or confused with local ledger identity | HIGH | Exact grant bindings; distinct Runner receipt ID; deduplicate identical receipt; reject conflicts; future authenticated envelope | Receipt conflict/replay alerts; ledger reconciliation | Digest-only ADR-0007 contract is not authenticity; envelope/trust design is OPEN |
...
- Cancellation before consume: receipt status `REJECTED`; cancellation after consume: the consumed
  attempt remains and postcondition verification selects receipt status `CANCELLED` or `FAILED`,
  using outcome and postcondition `INDETERMINATE` when the post-state is uncertain.
...
- Cancellation race tests at every state, including lost acknowledgements and unresponsive capsule.
