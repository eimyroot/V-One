...
curl --fail --silent --show-error http://127.0.0.1:18000/api/v1/health | \
  docker exec --interactive "$container" python -c 'import json,sys; data=json.load(sys.stdin); assert data["status"] == "HEALTHY"; assert data["database_backend"] == "sqlite"; assert data["schema_version"] == 7; assert data["production_effects"] == "DISABLED"'
