...
printf '%s\n' "$health_payload" | \
  docker exec --interactive "$container" python -c 'import json,sys; data=json.load(sys.stdin); assert data["status"] == "HEALTHY"; assert data["database_backend"] == "sqlite"; assert data["schema_version"] == 14; assert data["production_effects"] == "DISABLED"'
