...
def product_config(tmp_path: Path, *, name: str) -> ProductConfig:
...
def test_composed_services_share_one_product_database_and_evidence_authority(tmp_path: Path) -> None:
    app = FastAPI()
    composition = install_composed_product_platform(
        app,
        config=product_config(tmp_path, name="composition"),
    ...
    assert composition.session_lifecycle_service.db is composition.service.db
    ...
    assert composition.session_lifecycle_service.audit_ledger is composition.audit_ledger
    ...
    assert composition.receipt_ledger.audit_ledger is composition.audit_ledger
    assert composition.operational_safety_service.audit_ledger is composition.audit_ledger
...
def test_composed_services_operate_through_shared_ledger_and_external_identity_boundary(
    tmp_path: Path,
) -> None:
    app = FastAPI()
    composition = install_composed_product_platform(
        app,
        config=product_config(tmp_path, name="shared"),
        repository_root=tmp_path,
    )
    bootstrap = composition.service.bootstrap_admin(
        username="shared-admin",
        password="VeryStrongSharedAdminPassword1!",
        token="b" * 48,
    )
    administrator = composition.service.get_user(str(bootstrap["user_id"]))
...
def test_composed_installer_preserves_routes_and_middleware(tmp_path: Path) -> None:
    ...
    canonical_additions = set(composed_paths) - set(legacy_paths)
    assert canonical_additions == {
